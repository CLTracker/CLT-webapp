...
CREATE TABLE IF NOT EXISTS `mydb`.`conference` (
  ...
  INDEX `SECONDARY` (`conference_name` ASC))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `mydb`.`permitted_exhibitors` (
  ...
  INDEX `conference_pexhb_idx` (`conference_id` ASC),
  CONSTRAINT `conference_pexhb`
...
CREATE TABLE IF NOT EXISTS `mydb`.`permitted_organizers` (
  ...
  `conference_id` INT NOT NULL,
  PRIMARY KEY (`organizer_email`),
  INDEX `conference_idx` (`conference_id` ASC),
...
CREATE TABLE IF NOT EXISTS `mydb`.`news` (
  ...
  INDEX `news_conference_idx` (`conference` ASC),
  CONSTRAINT `news_conference`
    FOREIGN KEY (`conference`)
    REFERENCES `mydb`.`conference` (`conference_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
